...
def entscheide_richtung() -> int:
    return (
        0 if rd.random() < 0.5 else 1
    )  # rd.random() gibt eine Zufallszahl zwischen 0 und 1 zurück
...
def kugel_durchlauf(hoehe: int) -> int:
    ...
    for _ in range(
        hoehe
    ):  # _ wird als Platzhalter für Variablen verwendet, die nicht verwendet werden
...
def simuliere_galton_board(hoehe: int, anzahl_kugeln: int) -> np.ndarray:
    behaelter: np.ndarray = initialisiere_behaelter(
        hoehe
    )  # np.ndarray: n-dimensionales Array

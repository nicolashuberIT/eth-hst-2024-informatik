...
def burn(grid: np.ndarray, x: int, y: int) -> bool: # Prüft ob Baum in der Nähe brennt
    for dx in [-1, 0, 1]: # Schleife über die Nachbarzellen in x-Richtung (Schleife wird für jeden Wert in der Liste ausgeführt)
...
def update(grid: np.ndarray) -> np.ndarray: # Berechnet Zustände für nächste Zeiteinheit
    new_grid = grid.copy()

    for x in range(size):
        for y in range(size):

            if grid[x, y] == EMPTY and np.random.random() < p: # np.random.random() gibt eine Zufallszahl zwischen 0 und 1 zurück
                new_grid[x, y] = TREE
            elif grid[x, y] == TREE:

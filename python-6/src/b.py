...
def setup() -> np.ndarray:
    return np.full((3, 3), '.') # np.full() erstellt ein Array der Größe (3, 3) und füllt es mit '.'
...
def ausgabe(spielfeld: np.ndarray) -> None:
    ...
    for i in range(3):
        print(f"{i + 1} {' '.join(spielfeld[i])}") # join() fügt die Elemente eines Arrays mit einem Leerzeichen zusammen
...
def hat_gewonnen(spielfeld: np.ndarray, zeichen: str) -> bool:
    for i in range(3):
        ...
        if np.all(spielfeld[i, :] == zeichen):  # Zeilen, np.all() prüft, ob alle Elemente eines Arrays True sind
            return True
        
        if np.all(spielfeld[:, i] == zeichen):  # Spalten
            return True
        
    if np.all(np.diag(spielfeld) == zeichen):  # Diagonale, np.diag() gibt die Diagonale eines Arrays zurück

        return True
    
    if np.all(np.diag(np.fliplr(spielfeld)) == zeichen):  # Gegendiagonale, np.fliplr() spiegelt ein Array horizontal
...
def computer_zug(spielfeld: np.ndarray) -> Tuple[int, int]:
    freie_felder = np.argwhere(spielfeld == '.') # np.argwhere() gibt die Indizes von Elementen zurück, die True sind
    return tuple(rd.choice(freie_felder)) # rd.choice() wählt zufällig ein Element aus einem Array aus
